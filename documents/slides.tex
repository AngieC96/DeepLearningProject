...
\title{\vspace*{1.5cm}An overview of GNNs and their expressivity}
...
\begin{frame}{Aim of the project}
    ...
    \alert{Graph Neural Networks} can have a lot of meanings, there isn't just one architecture that can be recognized as “GNN”. We will try to understand the general, abstract structure of a GNN that is presented in Hamilton's book \cite{HamiltonGRLBook} and to shed light about the relational inductive bias (presented in Battaglia's paper \cite{battaglia2018relational}) and combinatorial generalization of a GNN (described in Cappart's paper \cite{cappart2021combinatorial}).
    ...
    We will first present the \alert{theory} behind GNN, to understand what they are capable of and their limitations, then we will show a \alert{practical implementation} to make a \emph{graph prediction}, using the library DGL (\href{https://docs.dgl.ai/en/0.6.x/index.html}{documentation}).
...
\begin{frame}{Generalized Message Passing}
    ...
    \begin{align}
        ...
        \label{eq:message} \\
        \mathbf h^{(k)}_u &= \textsc{update}_\text{node} \left( \mathbf h_u^{(k-1)}, \mathbf m_{\mathcal N(u)}^{(k)}, \mathbf h_G^{(k-1)} \right) \qquad &\forall u \in V \qquad
        \label{eq:upnode} \\
